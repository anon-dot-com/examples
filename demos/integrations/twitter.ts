...
export class TwitterActions {
  ...
  async toggleRetweet() {
    ...
    const isRetweeted =
      (await this.page.getByTestId("unretweet").first().count()) > 0;
    ...
    try {
      ...
      if (options?.text) {
        ...
        if (options.imageUrl) {
          ...
          const imageBuffer = await this.helperActions.downloadImage(
            options.imageUrl,
          );
          await this.helperActions.uploadImage(imageBuffer);
        }
  ...
  async follow() {
    try {
      ...
      const buttons = await this.page
        .locator('aside[aria-label="Relevant people"] button')
        .all();
      console.log(
        "Available buttons in Relevant people:",
        await Promise.all(buttons.map((b) => b.textContent())),
      );
      throw error;
    }
  }
  ...
  async toggleBookmark(shouldBookmark: boolean = true) {
    try {
      const isBookmarked =
        (await this.page.getByTestId("removeBookmark").count()) > 0;
